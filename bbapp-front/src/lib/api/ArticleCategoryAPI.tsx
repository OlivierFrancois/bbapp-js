...
export class ArticleCategoryAPI {
    ...
    static async get(id: number): Promise<ArticleCategory[]> {
        return api.get(URL_GET.replace(':categoryId', id.toString())).then((res) => {
            return res.data;
        });
    }
    ...
    static async update(category: ArticleCategory): Promise<ArticleCategory> {
        return api.put(URL_UPDATE.replace(':categoryId', category.id.toString()), { ...category }).then((res) => {
            return res.data;
        });
    }

    static async delete(categoryId: number): Promise<{ message: string }> {
        return api.delete(URL_DELETE.replace(':categoryId', categoryId.toString())).then((res) => {
            return res.data;
        });
    }
}
