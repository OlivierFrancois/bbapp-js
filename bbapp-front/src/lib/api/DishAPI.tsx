...
export class DishAPI {
    static async get(id: number): Promise<Dish[]> {
        return axios.get(URL_GET.replace(':dishId', id.toString())).then((res) => {
            return res.data;
        });
    }
    ...
    static async create(dish: Omit<Dish, 'id'>): Promise<Dish> {
        return axios.post(URL_CREATE, { ...dish }).then((res) => {
            return res.data;
        });
    }

    static async update(dish: Dish, recipeItems: RecipeItem[]): Promise<{ dish: Dish; recipeItems: RecipeItem[] }> {
        return axios.put(URL_UPDATE.replace(':dishId', dish.id.toString()), { ...dish, recipeItems }).then((res) => {
            return res.data;
        });
    }

    static async delete(dishId: number): Promise<{ message: string }> {
        return axios.delete(URL_DELETE.replace(':dishId', dishId.toString())).then((res) => {
            return res.data;
        });
    }
}
