...
export class RecipeAPI {
    static async get(dishId: number, articleId: number): Promise<RecipeItem | null> {
        const url = URL_GET.replace(':dishId', dishId.toString()).replace(':articleId', articleId.toString());
    ...
    static async save(recipeItem: RecipeItem): Promise<RecipeItem> {
        return axios.post(URL_SAVE, { ...recipeItem }).then((res) => res.data);
    }

    static async delete(dishId: number, articleId: number): Promise<void> {
        const url = URL_DELETE.replace(':dishId', dishId.toString()).replace(':articleId', articleId.toString());
