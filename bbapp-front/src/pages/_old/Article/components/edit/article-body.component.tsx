...
export default function ArticleBody() {
    ...
    const handleSaveArticle = () => {
        if (article.id > 0) {
            ArticleAPI.update(article).then((returnedArticle) => setSelectedArticle(returnedArticle));
        } else {
            const { id, ...newArticle } = article;
            ArticleAPI.create(newArticle).then((returnedArticle) => setSelectedArticle(returnedArticle));
        }
    };
    const handleDelete = () => {
        ArticleAPI.delete(selectedArticle.id).then(() => {
    ...
    return (
        <div className={'p-2 flex flex-col gap-3'}>
            <div className="flex items-center gap-2 justify-end">
                ...
                <button disabled={!hasChanged} onClick={handleSaveArticle} className={'btn btn-xs btn-primary'}>
                    Sauvegarder
                </button>
            </div>
                    ...
                    >
                        ...
                        {categories
                            ...
                            .map((category, keyCategory) => (
                                <option key={keyCategory} className={'first-letter:capitalize'} value={category.id}>
