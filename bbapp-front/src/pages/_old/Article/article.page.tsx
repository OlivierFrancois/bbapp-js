...
export default function ArticlePage() {
    ...
    useEffect(() => {
        ArticleAPI.getAll().then((fetchedArticles) => setArticles(fetchedArticles));
        ArticleCategoryAPI.getAll().then((fetchedCategories) => setCategories(fetchedCategories));
    ...
    return (
        <ArticleContext.Provider value={{ selectedArticle, setSelectedArticle, selectedCategory, setSelectedCategory, articles, categories }}>
            <div className={'h-screen flex flex-col gap-2 relative overflow-hidden w-full'}>
                <div
                    className={
                        'bg-gradient-to-br from-primary to-primary/85 from-10% text-secondary-content font-semibold text-xl px-2 py-1 h-12 flex justify-between items-center'
                    }
                >
