...
import { RecipeAPI } from '../../../../../lib/api/RecipeAPI.tsx';
...
export default function Body() {
    ...
    const recipeHasChanged = useCallback((): boolean => {
        ...
        for (const [key, cleanItem] of cleanMap.entries()) {
            ...
            if (!recipeItem || cleanItem.quantity !== recipeItem.quantity || cleanItem.unit !== recipeItem.unit) {
                return true;
            }
        }
        return false;
    }, [cleanRecipeItems, recipeItems]);
    const dishTagsHaveChanged = useCallback((): boolean => {
    ...
    useEffect(() => {
        if (dish && selectedDish) {
            setHasChanged(dish.name !== selectedDish.name || dish.url !== selectedDish.url || dishTagsHaveChanged() || recipeHasChanged());
        }
    ...
    const handleSaveDish = () => {
        if (dish.id > 0) {
            DishAPI.update(dish, recipeItems).then((response) => {
                setSelectedDish(response.dish);
            });
        } else {
            const { id, ...payload } = dish;
            DishAPI.create(payload as Omit<Dish, 'id'>).then((returnedDish) => setSelectedDish(returnedDish));
        }
    };
    ...
    const handleDelete = () => {
        DishAPI.delete(selectedDish.id).then(() => setSelectedDish(null));
    };
    ...
    return (
        <div className={'p-2 flex flex-col gap-3'}>
            <div className="flex items-center gap-2 justify-end">
                ...
                <button disabled={!hasChanged} onClick={handleSaveDish} className={'btn btn-xs btn-primary'}>
                    Sauvegarder
                </button>
            </div>
